...
export class LAppLive2DManager {
  ...
  public onUpdate(): void {
    ...
    const projection: CubismMatrix44.CubismMatrix44 = new CubismMatrix44.CubismMatrix44();
  ...
  public nextScene(): void {
    const no: number = (this._sceneIndex + 1) % this.modelNames.length;
    this.changeScene(no);
  }
  ...
  public changeScene(index: number): void {
    ...
    const model: string = this.modelNames[index];
  ...
  _viewMatrix: CubismMatrix44.CubismMatrix44; // モデル描画に用いるview行列
  _models: csmVector.csmVector<LAppModel>; // モデルインスタンスのコンテナ
  _sceneIndex: number; // 表示するシーンのインデックス値
  // モーション再生終了のコールバック関数
  _finishedMotion = (self: ACubismMotion.ACubismMotion): void => {
